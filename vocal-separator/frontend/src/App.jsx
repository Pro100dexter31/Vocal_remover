...
function App() {
  ...
  const renderLoadingState = () => (loading || taskId || (status !== 'IDLE' && status !== 'SUCCESS')) && (
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-slate-950 px-5 py-10 text-slate-100 sm:px-8"><div className="mx-auto max-w-4xl">
      ...
      <section className="glass-effect animate-slide-in-left rounded-3xl p-5 sm:p-8">{renderUploadZone()}{selectedFile && <AudioTrimmer file={selectedFile} onTrimChange={(start, end) => { setTrimStartSeconds(start); setTrimEndSeconds(end); }} onUpload={(start, end) => { setTrimStartSeconds(start); setTrimEndSeconds(end); uploadFile(selectedFile); }} />}{selectedFile && status === 'IDLE' && !loading && (<button type="button" onClick={() => uploadFile(selectedFile)} className="mt-6 w-full rounded-xl bg-primary-600 px-6 py-4 text-base font-semibold shadow-lg shadow-primary-950/40 hover:bg-primary-500">Separă acum</button>)}{error &&<p role="alert" className="mt-5 rounded-xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">{error}</p>}{renderLoadingState()}{renderSuccessState()}{(status === 'SUCCESS' || status === 'FAILED') && <button type="button" onClick={handleReset} className="mt-5 text-sm font-semibold text-slate-400 underline-offset-4 hover:text-white hover:underline">Process another file</button>}</section>
